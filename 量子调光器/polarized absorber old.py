...
nu_down = 508.480 # THz 
nu_up = 508.998 # THz 
nulist = np.hstack((np.linspace(508.45,508.51,300),np.linspace(508.515,508.965,50),np.linspace(508.97,509.04,300)))
...
def ab_main():
############################################################################ users set 
    N_ab = 7*1e17# number ddensity of sodium atom of absorbing
    T_ab = 800 # tempurature of Na absorber ###########************************************************************
    alpha = np.pi/5 # range in [0,pi], 
    ...
    G_emit_total = 4*(4*g_emit_up*lbd_up**2/(8*pi*tsp_up) + 8*g_emit_down*lbd_down**2/(8*pi*tsp_down))
    ...
    B = np.hstack((np.arange(0,0.3,0.05),np.arange(0.3,1.1,0.1))) 
    eta = np.empty_like(B)
    ...
    for k in range(len(B)):
        ...
        G_ab_L_total = 4*(g_ab_up_L*(c0_const/nu_up)**2/(8*pi*tsp_up) + g_ab_down_L*(c0_const/nu_down)**2/(8*pi*tsp_down))/1e24
        G_ab_R_total =  4*(g_ab_up_R*(c0_const/nu_up)**2/(8*pi*tsp_up) + g_ab_down_R*(c0_const/nu_down)**2/(8*pi*tsp_down))/1e24
        G_ab_pi_total = 4*(g_ab_up_pi*(c0_const/nu_up)**2/(8*pi*tsp_up) + g_ab_down_pi*(c0_const/nu_down)**2/(8*pi*tsp_down))/1e24
        
        G_ab_total = G_ab_pi_total*np.sin(alpha/2)**2 + (G_ab_L_total + G_ab_R_total)*np.cos(alpha/2)**2/2

        absorption_nu = N_ab*G_ab_total*d/1e12
        # total transmittance
        
        varphi_0 = nulist*G_emit_total
        varphi_t = nulist*G_emit_total*np.exp(-absorption_nu)
        
        I0,_ = ni.int_integral(varphi_0,nulist,order = 2,smooth = 0.01)
        I_transmit,_ = ni.int_integral(varphi_t,nulist,order = 2,smooth = 0.01)
    ...
    plt.figure(2)
    plt.plot(nulist,G_emit_total,label = 'emit line')
    plt.plot(nulist,G_ab_total,label = 'absorption line')
...
def ab_line(nu0,deltaG_nu,deltaL_nu):
    ...
    if type(nu0) == float:
        g = voigt_profile(nulist-nu0,deltaG_nu,deltaL_nu) 
    elif type(nu0) == np.ndarray:
        g = ab_line_func(nu0,deltaG_nu,deltaL_nu)
        
    return g 

def ab_line_func(nu0,deltaG_nu,deltaL_nu):
    g = 0
    
    if type(deltaG_nu) == np.ndarray:
        for i in range(len(nu0)):
            g = g + voigt_profile(nulist-nu0[i],deltaG_nu[i],deltaL_nu[i]) 
    else:
        for i in range(len(nu0)):
            g = g + voigt_profile(nulist-nu0[i],deltaG_nu,deltaL_nu)
